...
const MediaViewer: React.FC<MediaViewerProps> = ({ mediaItems, categoryName }) => {
  ...
  // Reset currentIndex về 0 mỗi khi categoryName thay đổi
  useEffect(() => {
    setCurrentIndex(0);
  }, [categoryName]);

  // Đảm bảo currentIndex không vượt quá số lượng ảnh
  const safeIndex = currentIndex >= mediaItems.length ? 0 : currentIndex;
  const currentItem = mediaItems[safeIndex];

  const goToPrevious = () => {
    setCurrentIndex(safeIndex === 0 ? mediaItems.length - 1 : safeIndex - 1);
  };

  const goToNext = () => {
    setCurrentIndex(safeIndex === mediaItems.length - 1 ? 0 : safeIndex + 1);
  };
  ...
  const renderMediaContent = () => {
    if (!currentItem) {
      return null; // Phòng trường hợp không có item nào
    }
        ...
        {mediaItems.map((item, index) => (
          <Box
            sx={{
              ...
              height: "12px",
              borderRadius: "50%",
              backgroundColor: index === safeIndex ? "#FFD700" : "#ccc",
              margin: "0 5px",
              cursor: "pointer",
              transition: "background-color 0.2s",
            }}
